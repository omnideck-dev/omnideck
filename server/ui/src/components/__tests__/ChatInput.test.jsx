...
describe('ChatInput', () => {
    ...
    describe('per-conversation drafts', () => {
        it('persists unsent text per conversation in the draft store', async () => {
            const user = userEvent.setup();
            const draftStore = { current: {} };
            render(<ChatInput onSend={vi.fn()} isStreaming={false} conversationId="A" draftStore={draftStore} />);

            await user.type(screen.getByPlaceholderText('Message Omnideck…'), 'hello A');

            expect(draftStore.current.A).toBe('hello A');
        });

        it('swaps the draft when the active conversation changes', async () => {
            const user = userEvent.setup();
            const draftStore = { current: { B: 'draft for B' } };
            const { rerender } = render(
                <ChatInput onSend={vi.fn()} isStreaming={false} conversationId="A" draftStore={draftStore} />,
            );

            await user.type(screen.getByPlaceholderText('Message Omnideck…'), 'typed in A');
            expect(screen.getByPlaceholderText('Message Omnideck…').value).toBe('typed in A');

            // Switching to B shows B's stored draft, not A's text.
            rerender(<ChatInput onSend={vi.fn()} isStreaming={false} conversationId="B" draftStore={draftStore} />);
            await waitFor(() => {
                expect(screen.getByPlaceholderText('Message Omnideck…').value).toBe('draft for B');
            });
            expect(draftStore.current.A).toBe('typed in A');

            // Switching back to A restores its preserved draft.
            rerender(<ChatInput onSend={vi.fn()} isStreaming={false} conversationId="A" draftStore={draftStore} />);
            await waitFor(() => {
                expect(screen.getByPlaceholderText('Message Omnideck…').value).toBe('typed in A');
            });
        });
        ...
        it('clears the stored draft after sending', async () => {
            ...
            const textarea = screen.getByPlaceholderText('Message Omnideck…');
            await user.type(textarea, 'send me');
            await user.click(screen.getByLabelText('Send message'));

            expect(draftStore.current.A).toBeUndefined();
        });
    });
});
