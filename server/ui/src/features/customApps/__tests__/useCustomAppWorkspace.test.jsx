...
function setup({ homeAppSlug = null } = {}) {
    ...
    return { result, preview, setDraft, navigation, onHomeAppChange };
}

afterEach(() => vi.restoreAllMocks());

test('owns app presentation while adapting conversation preview tabs', () => {
    const { result, preview, navigation } = setup({ homeAppSlug: 'text-lab' });
    ...
    expect(navigation.openWorkspace).toHaveBeenCalled();
...
test('keeps an app open across previews and closes it only from its own tab', () => {
    const { result, preview, navigation } = setup();
    ...
    expect(navigation.openChat).toHaveBeenCalled();
});
