 ...
 */
const ConversationCatalogContext = createContext(null);

export function ConversationCatalogProvider({ children }) {
    ...
    return (
        <ConversationCatalogContext.Provider value={value}>
            {children}
        </ConversationCatalogContext.Provider>
    );
}

/** Returns the recent-conversations list and its mutators. Throws outside the provider. */
export function useConversationCatalog() {
    const value = useContext(ConversationCatalogContext);
    if (value === null) {
        throw new Error('useConversationCatalog must be used inside <ConversationCatalogProvider>');
    }
    return value;
}
