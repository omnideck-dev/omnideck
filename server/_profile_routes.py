...
from sdk.skills._policy import strip_internal_skills
...
def _normalize_browser_settings(body: dict) -> None:
    body["skills"] = strip_internal_skills(body.get("skills", []))
    if not body.get("browser_access"):
        body["browser_profile_id"] = None
    profile_id = body.get("browser_profile_id")
    if body.get("browser_access") and profile_id:
        from browser_profiles import get_browser_profile_store

        get_browser_profile_store().get(str(profile_id))
