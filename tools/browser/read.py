"""Agent tool for reading a tab as clean markdown."""
...
def _status_line(chunk: int, total_chunks: int, total_chars: int) -> str:
    ...
    if chunk < total_chunks:
        line += f'\n[Read on with chunk={chunk + 1}, or jump straight to a passage with query="...".]'
    return line


def _search(content: str, query: str, bounds: list[tuple[int, int]]) -> tuple[str, bool]:
    ...
    *text* holds the matching passages, each carrying a line of context either
    ...
    for run in runs:
        line_indices = [i for i in run if lines[i].strip()]
        if not line_indices:
            continue
        text = "\n".join(lines[i] for i in range(line_indices[0], line_indices[-1] + 1))
    ...
    header = f'[Search "{query}" — {len(passages)} match(es) in {where} of {len(bounds)}.]'
    capped = len(shown) < len(passages)
    if capped:
        header += f"\n[Showing the first {len(shown)}. Narrow the query, or read a chunk to see a match in full.]"
    else:
...
async def read_page(
    ...
    *,
    ...
    Args:
        ...
        tab: Stable tab ID to read — the ID shown in the document
            header (e.g. ``tab="3"``).
    ...
    _browser, resolved_tab, document = await get_document("read_page", tab=tab)
    header = format_page_header(
        title=await resolved_tab.title(),
        url=resolved_tab.url,
        tab_id=resolved_tab.id,
    )

    # A blocked document has no readable text in the browser; return only the banner
    # (which routes the agent to fetch_url) and skip the read entirely.
    if resolved_tab.challenge:
        return f"{header}\n\n{resolved_tab.challenge.banner}"

    try:
        raw_html: str = await document.content()
    except PlaywrightError as exc:
        logger.exception("Failed to read page content")
        raise BrowserToolError("Failed to read page content", tool="read_page") from exc
    ...
    if query:
        ...
        if not content:
            content = (
                f'[Search "{query}" — no matches on this page ({len(full_content):,} chars, {len(bounds)} chunks).]'
            )
    else:
        if chunk > len(bounds):
            raise BrowserToolError(
                f"chunk {chunk} is past the end of the page, which has {len(bounds)}",
                tool="read_page",
            )
