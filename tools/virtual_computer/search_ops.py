...
async def grep(
    ...
    *,
    ...
    Args:
        ...
        context: Lines before and after each match, from 0 to 20.
        max_results: Requested match limit, from 1 to 10000. None uses the hard limit of 10000.
    """
    if len(pattern) > 4096 or not 0 <= context <= 20 or (max_results is not None and not 1 <= max_results <= 10_000):
        return GrepResult(
            success=False, matches=[],
            error="Use a pattern of at most 4096 characters, context from 0 to 20, and max_results from 1 to 10000 (or null).",
        )
    arguments = dict(
