...
async def test_combobox_container_exposes_inner_input(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/combobox/search.html")
...
async def test_fill_field_types_into_combobox_input(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/combobox/search.html")
    view = await browse_page(tab=tab)
