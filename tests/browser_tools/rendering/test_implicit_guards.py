...
async def test_wrapper_with_interactive_child_not_double_emitted(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/nested-clickable/page.html")
...
async def test_image_only_clickable_is_promoted(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/nested-clickable/page.html")
    view = await browse_page(tab=tab)
