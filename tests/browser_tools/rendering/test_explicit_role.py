...
from tools.browser import fill_field
from tools.browser import browse_page

from .._helpers import find_ref


async def test_invalid_explicit_role_falls_back_to_implicit(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/explicit-role/page.html")
...
async def test_valid_explicit_role_preserved(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/explicit-role/page.html")
    view = await browse_page(tab=tab)
