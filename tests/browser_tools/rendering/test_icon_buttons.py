...
async def test_aria_labelled_icon_button_is_exposed(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/icon-buttons/page.html")
    view = await browse_page(tab=tab)
    assert find_ref(view, role="button", name="Labelled icon") is not None


async def test_title_only_icon_button_is_exposed(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/icon-buttons/page.html")
    view = await browse_page(tab=tab)
    assert find_ref(view, role="button", name="Titled icon") is not None


async def test_svg_title_icon_button_is_exposed(open_tab, servers):
    tab = await open_tab(f"{servers.primary}/icon-buttons/page.html")
    view = await browse_page(tab=tab)
