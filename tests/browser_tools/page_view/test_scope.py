...
async def test_scope_narrows_to_section(browser_session, servers):
    tab = await browser_session.open(f"{servers.primary}/scope/page.html")
...
async def test_scope_not_found_shows_full_page(browser_session, servers):
    tab = await browser_session.open(f"{servers.primary}/scope/page.html")
