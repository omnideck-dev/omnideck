...
async def test_drawer_below_fold_kept_background_dropped(browser_session, servers):
    ...
    tab = await browser_session.open(
        f"{servers.primary}/aria-hidden/app-hidden-drawer-below-fold.html"
    )
...
async def test_portal_dialog_in_zero_height_root(browser_session, servers):
    ...
    # of the page. The dialog's input and buttons were lost with it.
    tab = await browser_session.open(f"{servers.primary}/aria-hidden/portal-dialog.html")
    view = await browse_page(tab=tab)
