...
async def test_closed_summary_is_interactive_and_collapsed(browser_session, servers):
    tab = await browser_session.open(f"{servers.primary}/disclosure/page.html")
...
async def test_clicking_summary_expands_body(browser_session, servers):
    tab = await browser_session.open(f"{servers.primary}/disclosure/page.html")
    view = await browse_page(tab=tab)
