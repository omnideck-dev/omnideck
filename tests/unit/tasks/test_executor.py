...
RuntimeError rather than failing confusingly downstream at goal-run time.
"""
...
def test_profile_for_raises_when_task_has_no_profile() -> None:
    task = Task(goal_id="g1", description="orphan task", instruction="do the thing", agent_profile=None)
...
def test_profile_for_raises_when_profile_unknown(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    task = Task(goal_id="g1", description="task", instruction="prompt", agent_profile="does_not_exist")
    executor = TaskExecutor(store=None)  # type: ignore[arg-type]
