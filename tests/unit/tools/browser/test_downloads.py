...
class TestSaveResponseAsFile:
    ...
    async def test_refetches_when_viewer_html(self, tmp_path: Path) -> None:
        ...
        viewer_html = (
            b"<!doctype html><html><body><embed name='X' src='about:blank' type='application/pdf'></body></html>"
        )
...
class TestGotoFileDetection:
    ...
    async def test_goto_detects_pdf(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        mock_tab = MagicMock(id=1, url="about:blank")
        result = ActionResult(
            tab=mock_tab,
        ...
        mock_browser.get_tab = lambda tab: mock_tab
    ...
    async def test_goto_normal_html(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        mock_tab = MagicMock(id=1, url="https://example.com")
        mock_tab.render_document = AsyncMock(return_value=snapshot)
        result = ActionResult(
            tab=mock_tab,
            navigation_response=None,
            download=None,
...
# ---------------------------------------------------------------------------
# format_action_result file detection
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestFormatResultFileDetection:
    """Tests for format_action_result detecting downloads."""
    ...
    async def test_format_action_result_returns_download(self) -> None:
        ...
        output = await format_action_result(result, tool_name="goto")
    ...
    async def test_format_action_result_normal_page(self) -> None:
        ...
        result = ActionResult(tab=mock_tab)
