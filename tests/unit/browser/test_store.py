...
from browser.profile_store import BrowserProfileStore, summarize_browser_sites
...
@pytest.mark.parametrize("name", ["Empty", " empty ", "EMPTY"])
def test_empty_profile_name_is_reserved(tmp_path, name):
    store = BrowserProfileStore(tmp_path)

    with pytest.raises(ValueError, match="Empty is reserved"):
        store.create(name=name, icon="bi-globe2", storage_state=state())

    store.ensure_default()
    with pytest.raises(ValueError, match="Empty is reserved"):
        store.update_metadata("default", name=name)
