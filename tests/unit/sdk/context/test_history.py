...
class TestSystemMessage:
    ...
    def test_extracted_from_initial_messages(self):
        h = ConversationHistory([{"role": "system", "content": "sys"}])
        assert h.system_message == {"role": "system", "content": "sys"}

    def test_non_system_initial_messages_are_ignored(self):
        """Only the system message is honored at construction; others come from events."""
        h = ConversationHistory([
...
class TestDerivedMessages:
    ...
    def test_messages_prepends_system_to_derived(self):
        h = ConversationHistory(
            [{"role": "system", "content": "sys"}], conversation_id="test",
        )
        h.seed_events([_agent_started(), _user("hi"), _iter("hello")])
...
def _seed_root_agent(h: ConversationHistory, agent_id: str = "root.computron.1") -> str:
    ...
    root agent and build_llm_history will include them."""
