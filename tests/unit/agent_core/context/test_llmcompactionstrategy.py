...
async def test_compaction_chunk_size_uses_discovered_model_context():
    ...
    strategy = LLMCompactionStrategy(summary_model="large-context-model")
    messages = [{"role": "user", "content": "x" * 40_000}]

    with patch.object(strategy, "_call_summarizer", new_callable=AsyncMock) as call, \
         patch("agent_runtime._compaction.load_settings", return_value={
             "compaction_provider": "test-provider",
             "compaction_model": "large-context-model",
             "compaction_options": {},
         }), \
         patch("agent_runtime._compaction.get_provider", return_value=object()), \
         patch(
             "agent_core.providers.resolve_model_info",
...
async def test_intent_extraction_only_sets_supported_temperature(
    ...
    strategy = LLMCompactionStrategy(summary_model="test-model")
    provider = MagicMock()
    ...
    with patch("agent_runtime._compaction.load_settings", return_value={
        "compaction_provider": "test-provider",
        "compaction_model": "test-model",
        "compaction_options": {},
    }), patch("agent_runtime._compaction.get_provider", return_value=provider), patch(
        "agent_core.providers.resolve_role_options",
        new_callable=AsyncMock,
