...
def _events_first_test_bridge():
    """Override the parent agent core conftest's auto-init patching.
...
class TestSeedAndHandle:
    ...
    def test_handle_event_appends_to_log(self):
        from agent_core.events import AgentEvent, UserMessagePayload
    ...
    def test_handle_event_filters_transient_types(self):
        ...
        from agent_core.events import AgentEvent, ContentPayload, ToolCallPayload
        h = ConversationHistory(conversation_id="test", agent_id="test-agent")
...
def _seed_root_agent(h: ConversationHistory, agent_id: str = "root.computron.1") -> str:
    ...
    from agent_core.events import AgentStartedPayload
...
def test_add_event_filters_transient_types_from_in_memory_log():
    ...
    nor a warm resume."""
    from agent_core.events import ContentPayload
    h = ConversationHistory(conversation_id="c1")
...
def test_add_event_retains_warm_resume_types():
    ...
    from agent_core.events import FileOutputPayload, SpawnRequestedPayload
    h = ConversationHistory(conversation_id="c1")
