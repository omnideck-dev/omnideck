...
def _events_first_test_bridge(monkeypatch):
    ...
    via monkeypatch.
    """
    from agent_core.context._history import ConversationHistory
    ...
    # ``agent_core.events`` package re-export covers lazy-import sites
    # (notably ``agent_core.tools._helpers``).
    import agent_core.events as _events
    ...
    for modname in (
        "agent_core.hooks._nudge_hook",
        "agent_core.hooks._loop_detector",
        "agent_core.hooks._stop_hook",
        "agent_core.hooks._budget_guard",
    ):
        try:
