...
def _complete_setup_wizard(browser, wizard_choices):
    ...
    # Step 0: Welcome. Matched by role — the seeded welcome conversation puts the
    # same words in a sidebar item, which a plain text match would also pick up.
    page.get_by_role("heading", name="Welcome to Omnideck").wait_for(
    ...
    page.get_by_role("heading", name="Welcome to Omnideck").wait_for(state="hidden")
