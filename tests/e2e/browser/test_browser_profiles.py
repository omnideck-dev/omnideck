...
def _start_empty(api_client: ApiClient) -> None:
    response = api_client.post("/api/browser/session/load", data={"profile_id": "empty"})
...
def test_takeover_from_empty_can_save_and_assign_new_profile(page: Page, api_client: ApiClient):
    """Takeover snapshots stay unassigned unless the user confirms assignment."""
    ...
    try:
        ...
        modal.get_by_label("Profile name").fill("Takeover copy E2E")
        expect(modal.get_by_label("Use this profile for Empty Browser E2E next time")).not_to_be_checked()
        modal.get_by_test_id("browser-save-confirm").click()
        modal.wait_for(state="hidden")

        unassigned_profile = next(
            profile
            for profile in api_client.get("/api/browser/profiles").json()
            if profile["name"] == "Takeover copy E2E"
        )
        created_profile_ids.append(unassigned_profile["id"])
        assert api_client.get(f"/api/profiles/{agent_id}").json()["browser_profile_id"] == "empty"
        ...
        assert saved_agent["browser_profile_id"] not in (None, "empty")
        created_profile_ids.append(saved_agent["browser_profile_id"])
        saved_profile = next(
            profile
            for profile in api_client.get("/api/browser/profiles").json()
            if profile["id"] == saved_agent["browser_profile_id"]
        )
        assert saved_profile["name"] == "Takeover E2E"
    finally:
        api_client.delete(f"/api/profiles/{agent_id}")
        for created_profile_id in created_profile_ids:
...
def test_live_agent_browser_blocks_profile_deletion_until_conversation_closes(
    page: Page,
    api_client: ApiClient,
):
    """Conversation cleanup releases a profile after its live Browser closes."""
    profile = _save_new_profile(
        page,
        api_client,
        name="Live Browser deletion guard E2E",
        value="live-delete-guard",
    )
    _start_empty(api_client)
    agent_id = f"live_browser_agent_{uuid4().hex}"
    conversation_id = f"live_browser_conversation_{uuid4().hex}"
    _create_browser_agent(
        api_client,
        agent_id=agent_id,
        name="Live Browser Agent E2E",
    ...
    try:
        run = api_client.post(
            ...
            data={
                ...
                "profile_id": agent_id,
                "message": open_url(fixture_url("profile-report")),
            },
            timeout=30,
        )
        assert run.status == 200, run.text

        deleted_agent = api_client.delete(f"/api/profiles/{agent_id}")
        assert deleted_agent.status == 204
        agent_deleted = True

        blocked = api_client.delete(f"/api/browser/profiles/{profile['id']}")
        assert blocked.status == 409
        assert blocked.json()["usage"] == {
        ...
        deleted_conversation = api_client.delete(
            f"/api/conversations/sessions/{conversation_id}",
        )
        assert deleted_conversation.status == 204
        conversation_deleted = True

        removed = api_client.delete(f"/api/browser/profiles/{profile['id']}")
        assert removed.status == 204
