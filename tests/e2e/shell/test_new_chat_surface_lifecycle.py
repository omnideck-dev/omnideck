...
Browser and Terminal execution tabs are conversation-bound and covered in the
...
def test_new_chat_keeps_open_artifact_until_user_closes_it(page: Page):
    ...
    try:
        ...
        expect(
            page.locator("[data-surface-kind='artifact-file']")
        ).to_have_count(1)

        page.get_by_test_id(f"close-surface-tab-artifact:{name}").click()
        expect(tab).to_have_count(0)
        expect(page.locator("[data-surface-kind='artifact-file']")).to_have_count(0)
    finally:
        container_exec(
            "import pathlib\n"
