...
def test_terminal_persists_across_turns(page: Page):
    ...
    # Turn 1
    chat.send(bash('echo "turn-one"')).wait_streaming(timeout=LLM_TIMEOUT)
    assert chat.preview.terminal_tab.is_visible(), (
        "Terminal tab should appear after turn 1"
    )

    # Turn 2
    chat.send(bash('echo "turn-two"')).wait_streaming(timeout=LLM_TIMEOUT)
...
def test_new_conversation_clears_previews(page: Page):
    ...
    chat.send(bash('echo "before-reset"')).wait_streaming(timeout=LLM_TIMEOUT)
...
def test_new_conversation_allows_fresh_previews(page: Page):
    ...
    chat.send(bash('echo "old"')).wait_streaming(timeout=LLM_TIMEOUT)

    chat.new_conversation()

    chat.send(bash('echo "fresh"')).wait_streaming(timeout=LLM_TIMEOUT)
