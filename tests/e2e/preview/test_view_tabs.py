...
Verifies that output views appear in a normal tab group stack, can be switched
...
def test_output_pane_visible(preview_page: Page):
    """The second tab group is mounted when output tabs exist."""
    expect(ChatView(preview_page).preview.root).to_be_visible()


def test_split_handle_visible(preview_page: Page):
    """The split handle is visible when both tab groups contain tabs."""
...
def test_close_all_tabs_hides_second_pane(preview_page: Page):
    """After closing all its tabs, the second tab group and divider disappear."""
    preview = ChatView(preview_page).preview
