...
class CompactionAudit(BaseModel):
    """Audit fields recorded on a compaction event for forensics.

    Attributes:
        model: Summarizer model used.
...
class CompactionStats(BaseModel):
    ...
    Attributes:
        ...
        scope: Counts of what was compacted (user messages, iterations,
            tool results).
        sub_agents_spawned: Sub-agents spawned during the compacted range.
...
class CompactionPayload(BaseModel):
    ...
    Attributes:
        ...
        audit: Forensic fields about the summarizer call.
        stats: Presentation stats computed at compaction time for the
            UI chip panel. Optional — older compaction events emitted
            before this field existed leave it unset and the UI falls
            back to deriving from the event log.
    """
