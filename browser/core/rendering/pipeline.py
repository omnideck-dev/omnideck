"""Process structured DOM nodes into an agent-facing rendered document.

Transforms raw node data from the JavaScript DOM walk into the annotated text
...
Viewport filtering happens during the JavaScript DOM walk, which never emits an
off-screen element, so there is nothing left to filter here.
"""

from __future__ import annotations

from browser.core.rendering.filters import filter_for_site
from browser.core.rendering.nodes import (
    DomNode,
    NodeType,
    parse_nodes,
)

__all__ = ["render_nodes"]
...
def render_nodes(
    ...
    *,
    ...
    """Render structured DOM nodes as agent-facing annotated text.

    Args:
        raw_nodes: Raw node dicts from the JavaScript DOM walk.
...
def _filter_scope(
    ...
    if nodes[container_idx].type == NodeType.CONTAINER_START:
        ...
        for i in range(container_idx + 1, len(nodes)):
            node = nodes[i]
            if node.type == NodeType.CONTAINER_END and node.depth == container_depth:
                end_idx = i + 1
                break
